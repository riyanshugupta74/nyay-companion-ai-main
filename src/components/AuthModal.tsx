  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        // Check for duplicate email and mobile before registration
        const { data: duplicateCheck, error: checkError } = await supabase.functions.invoke('check-duplicate-registration', {
          body: { email, mobile }
        });

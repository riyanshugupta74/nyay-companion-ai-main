          ...
          >
            {question.options?.map((option, idx) => (
              <Label
                ...
                className={`flex items-center gap-2 p-3 rounded-lg border cursor-pointer transition-all ${value === option ? "border-nyay-teal bg-nyay-teal/5" : "border-border hover:border-nyay-teal/50"
                  }`}
              >
            ...
            <div className="flex justify-between">
              {[1, 2, 3, 4, 5].map((num) => (
                <button
                  ...
                  className={`w-8 h-8 rounded-full text-sm font-medium transition-all ${scaleValue === num
        ...
        <Card className="shadow-card">
          ...
          <CardContent className="space-y-6">
            <RadioGroup value={proceedChoice} onValueChange={setProceedChoice} className="space-y-3">
              <Label
                htmlFor="online"
                className={`flex items-start gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${proceedChoice === "online" ? "border-nyay-teal bg-nyay-teal/5" : "border-border hover:border-nyay-teal/50"
                  }`}
              >
              ...
              <Label
                htmlFor="lawyer"
                className={`flex items-start gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${proceedChoice === "lawyer" ? "border-nyay-gold bg-nyay-gold/5" : "border-border hover:border-nyay-gold/50"
                  }`}
              >
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-6">
          ...
          <Card className="shadow-card border-l-4 border-l-nyay-teal">
            <CardContent className="p-6">
              <div className="flex items-start gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${analysis.urgencyLevel === "high" ? "bg-destructive/10" :
                    analysis.urgencyLevel === "medium" ? "bg-nyay-gold/10" : "bg-nyay-teal/10"
                  }`}>
                  <CheckCircle className={`w-6 h-6 ${analysis.urgencyLevel === "high" ? "text-destructive" :
                ...
                <div>
                  ...
                  <div className="flex gap-3 mt-3">
                    <span className={`text-xs px-2 py-1 rounded-full ${analysis.urgencyLevel === "high" ? "bg-destructive/10 text-destructive" :
...
// NyayScan is a multi-step AI legal assistant UI that:

// Takes a user’s legal problem (text input)
// Sends it to AI (Supabase Edge Function)
// Shows basic analysis
// Optionally collects extra info via smart questions
// Accepts file uploads (documents, images, PDFs)
// Generates a detailed legal report
// Optionally suggests lawyers



// 🧑 User → “I have a legal issue”
// 🤖 AI → “Here’s what it is + what you should do + laws + steps + risks”